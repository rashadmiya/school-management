// routes/paymentRoutes.js
const express = require('express');
const router = express.Router();
const PaymentController = require('../PaymentController');
const { body, param } = require('express-validator');
const { isAuthenticated, authorizeRoles } = require('../../middleware/auth');
const catchAsyncErrors = require('../../middleware/catchAsyncErrors');

router.post('/',
    isAuthenticated,
    authorizeRoles('admin', 'accountant', 'cashier'),
    [
        body('studentId').isMongoId(),
        body('amount').isFloat({ min: 1 }),
        body('method').isIn(['cash', 'bank_transfer', 'check', 'mobile_banking', 'card', 'online']),
        body('session').optional().isString(),
    ],
    catchAsyncErrors(PaymentController.receivePayment)
);

router.post('/:id/void',
    isAuthenticated,
    authorizeRoles('admin', 'accountant'),
    [param('id').isMongoId(), body('reason').notEmpty().trim()],
    catchAsyncErrors(PaymentController.voidPayment)
);

router.get('/search',
    isAuthenticated,
    catchAsyncErrors(PaymentController.searchPayments)
);

router.get('/student/:studentId',
    isAuthenticated,
    param('studentId').isMongoId(),
    catchAsyncErrors(PaymentController.getPaymentHistory)
);

router.get('/:id/allocations',
    isAuthenticated,
    param('id').isMongoId(),
    catchAsyncErrors(PaymentController.getPaymentAllocations)
);

router.get('/student/:studentId/advance',
    isAuthenticated,
    param('studentId').isMongoId(),
    catchAsyncErrors(PaymentController.getAdvanceBalance)
);

router.post('/use-advance',
    isAuthenticated,
    authorizeRoles('admin', 'accountant'),
    [
        body('studentId').isMongoId(),
        body('feeInstanceId').isMongoId(),
        body('amount').isFloat({ min: 1 }),
    ],
    catchAsyncErrors(PaymentController.useAdvanceBalance)
);

router.post('/auto-apply-advance',
    isAuthenticated,
    authorizeRoles('admin', 'accountant'),
    [body('studentId').isMongoId()],
    catchAsyncErrors(PaymentController.autoApplyAdvance)
);

module.exports = router;

// // routes/paymentRoutes.js — cleaned
// const express = require('express');
// const router = express.Router();
// const PaymentController = require('../PaymentController');
// const { body, param } = require('express-validator');
// const { isAuthenticated, authorizeRoles } = require('../../middleware/auth');
// const catchAsyncErrors = require('../../middleware/catchAsyncErrors');

// // Receive Payment
// router.post('/',
//     isAuthenticated,
//     authorizeRoles('admin', 'accountant', 'cashier'),
//     [
//         body('studentId').isMongoId(),
//         body('amount').isFloat({ min: 1 }),
//         body('method').isIn(['cash', 'bank_transfer', 'check', 'mobile_banking', 'card', 'online']),
//         body('session').optional().isString(),
//     ],
//     catchAsyncErrors(PaymentController.receivePayment)
// );

// // Void Payment
// router.post('/:id/void',
//     isAuthenticated,
//     authorizeRoles('admin', 'accountant'),
//     [
//         param('id').isMongoId(),
//         body('reason').notEmpty().trim(),
//     ],
//     catchAsyncErrors(PaymentController.voidPayment)
// );

// // Payment History
// router.get('/student/:studentId',
//     isAuthenticated,
//     param('studentId').isMongoId(),
//     catchAsyncErrors(PaymentController.getPaymentHistory)
// );

// // Search
// router.get('/search',
//     isAuthenticated,
//     catchAsyncErrors(PaymentController.searchPayments)
// );

// // Allocations
// router.get('/:id/allocations',
//     isAuthenticated,
//     param('id').isMongoId(),
//     catchAsyncErrors(PaymentController.getPaymentAllocations)
// );

// // Advance
// router.get('/student/:studentId/advance',
//     isAuthenticated,
//     param('studentId').isMongoId(),
//     catchAsyncErrors(PaymentController.getAdvanceBalance)
// );

// router.post('/use-advance',
//     isAuthenticated,
//     authorizeRoles('admin', 'accountant'),
//     [
//         body('studentId').isMongoId(),
//         body('feeInstanceId').isMongoId(),
//         body('amount').isFloat({ min: 1 }),
//     ],
//     catchAsyncErrors(PaymentController.useAdvanceBalance)
// );

// router.post('/auto-apply-advance',
//     isAuthenticated,
//     authorizeRoles('admin', 'accountant'),
//     [body('studentId').isMongoId()],
//     catchAsyncErrors(PaymentController.autoApplyAdvance)
// );

// module.exports = router;

// // routes/paymentRoutes.js
// const express = require('express');
// const router = express.Router();
// const PaymentController = require('../PaymentController');
// const { body, param, query } = require('express-validator');

// const { isAuthenticated, authorizeRoles, isStudentAuthenticated } = require('../../middleware/auth');

// // Receive Payment
// router.post('/',
//     isAuthenticated,
//     [
//         body('studentId').isMongoId(),
//         body('amount').isFloat({ min: 1 }),
//         body('method').isIn(['cash', 'bank_transfer', 'check', 'mobile_banking', 'card', 'online']),
//         body('session').optional().isString()
//     ],
//     PaymentController.receivePayment
// );

// // Payment History
// router.get('/student/:studentId',
//     // isStudentAuthenticated,
//     isAuthenticated,
//     param('studentId').isMongoId(),
//     PaymentController.getPaymentHistory
// );

// // ADD THIS NEW ROUTE
// router.get('/search',
//     isAuthenticated,
//     PaymentController.searchPayments
// );

// // Payment Allocations
// router.get('/:id/allocations',
//     isAuthenticated,
//     param('id').isMongoId(),
//     PaymentController.getPaymentAllocations
// );

// // Advance Balance
// router.get('/student/:studentId/advance',
//     // isStudentAuthenticated,
//     param('studentId').isMongoId(),
//     PaymentController.getAdvanceBalance
// );

// router.post('/use-advance',
//     isAuthenticated,
//     authorizeRoles('admin', 'accountant'),
//     [
//         body('studentId').isMongoId(),
//         body('feeInstanceId').isMongoId(),
//         body('amount').isFloat({ min: 1 })
//     ],
//     PaymentController.useAdvanceBalance
// );

// router.post('/auto-apply-advance',
//     isAuthenticated,
//     authorizeRoles('admin', 'accountant'),
//     [
//         body('studentId').isMongoId()
//     ],
//     PaymentController.autoApplyAdvance
// );

// router.post('/payments',
//     isAuthenticated,
//     authorizeFinance(['admin', 'accountant', 'cashier']),
//     catchAsyncErrors(PaymentController.receivePayment)
// );

// router.post('/payments/:id/void',
//     isAuthenticated,
//     authorizeFinance(['admin', 'accountant']),
//     catchAsyncErrors(PaymentController.voidPayment)
// );
// module.exports = router;