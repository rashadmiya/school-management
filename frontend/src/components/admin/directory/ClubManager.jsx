...
const useTheme = () => {
    ...
    return {
        ...
        schedule: {
            day: isDarkMode ? "text-gray-300" : "text-gray-700",
            time: isDarkMode ? "text-gray-400" : "text-gray-500",
        },
    };
};
        ...
        return (
            <Card className={`${theme.bgCard} ${theme.border} shadow-sm`}>
                <CardContent className="p-6">
                    <div className={`text-center ${theme.textMuted}`}>Loading clubs data...</div>
                </CardContent>
            </Card>
        );
    }
            ...
            <Card className={`${theme.bgCard} ${theme.border} shadow-sm`}>
                <CardHeader>
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            <CardTitle className={`text-2xl ${theme.text}`}>Clubs Management</CardTitle>
            ...
            <Card className={`${theme.bgCard} ${theme.border} shadow-sm`}>
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        {/* Supervisor */}
                        <div className="space-y-2">
                            <label className={`text-sm font-medium ${theme.textSecondary}`}>Supervisor</label>
                            <Select 
                            ...
                            >
                                ...
                                <SelectContent className={theme.select.content}>
                                    <SelectItem value="all" className={theme.select.item}>All Supervisors</SelectItem>
                    ...
                    <div className="overflow-x-auto">
                        <Table>
                            <TableHeader className={`${theme.tableHeader} border-b ${theme.border}`}>
                                <TableRow>
                                    <TableHead className={theme.textSecondary}>Club Name</TableHead>
                                    <TableHead className={theme.textSecondary}>Supervisor</TableHead>
                                    <TableHead className={theme.textSecondary}>Members</TableHead>
                                    <TableHead className={theme.textSecondary}>Meeting Schedule</TableHead>
                            ...
                            <TableBody>
                                {clubs.length === 0 ? (
                                    ...
                                    clubs.map((club) => (
                                        <TableRow key={club._id} className={`border-b ${theme.tableRow}`}>
                                            <TableCell>
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${theme.avatar.bg}`}>
                                                        <BookOpen className={`w-5 h-5 ${theme.avatar.text}`} />
                                                    </div>
                                                    <div>
                                                        <div className={`font-medium ${theme.text}`}>{club.clubName}</div>
                                                        {club.description && (
                                                            <div className={`text-xs ${theme.textMuted} truncate max-w-[200px]`}>
                                                                {club.description}
                                                            </div>
                                                        )}
                                                    </div>
                                            ...
                                            <TableCell>
                                                {club.meetingSchedule ? (
                                                    <div className="space-y-1">
                                                        <div className={`flex items-center gap-1 text-sm ${theme.schedule.day}`}>
                                                            <Calendar className="w-3 h-3" />
                                                            {club.meetingSchedule.day}
                                                        </div>
                                                        <div className={`flex items-center gap-1 text-xs ${theme.schedule.time}`}>
                                                    ...
                                                    <span className={`text-sm ${theme.textMuted}`}>Not scheduled</span>
                                                )}
                                            </TableCell>
                                            ...
                                            <TableCell>
                                                <Badge 
                                                    variant={club.isActive ? "default" : "outline"}
                                                    className={club.isActive ? theme.badge.active : theme.badge.inactive}
                                                >
