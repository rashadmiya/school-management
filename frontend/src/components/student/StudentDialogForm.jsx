...
import TempPinDialog from "../common/TempPinDialog";
...
export default function StudentDialogForm({
    ...
    const onSubmit = async (formData) => {
        try {
            ...
            if (initialData) {
                response = await updateStudent({ id: initialData._id, ...submitData }).unwrap();
                if (response.tempPin) {
                    setTempPinData({
                        studentName: submitData.name,
                ...
                if (!submitData.password) submitData.password = "123456";

                if (photo) {
                    const formDataWithPhoto = new FormData();
                    Object.keys(submitData).forEach((key) => {
                        if (key === "lastExamResult" || Array.isArray(submitData[key])) {
                            formDataWithPhoto.append(key, JSON.stringify(submitData[key]));
                        } else {
                            formDataWithPhoto.append(key, submitData[key]);
                        }
                    ...
                    formDataWithPhoto.append("photo", photo);
                    response = await createStudentWithPhoto(formDataWithPhoto).unwrap();
                } else {
                    response = await createStudent(submitData).unwrap();
                }
                ...
                if (response.tempPin) {
                    ...
                    toast.success(`Linked to existing parent (${response.parent.name})`);
                } else {
                    toast.info("Student created without parent association");
                }
            }

            // Don't close the dialog yet if we're showing a temp PIN.
            // The TempPinDialog will handle closing.
            if (!response.tempPin) {
                onOpenChange(false);
                onSaved?.();
            } else {
    ...
    const onError = (errors) => {
        const fieldToTab = {
            name: "basic", rollNumber: "basic", session: "basic", password: "basic",
            guardianContact: "family", fathersName: "family", mothersName: "family",
            classId: "academic",
            gender: "personal", dateOfBirth: "personal", religion: "personal",
        };
        const first = Object.keys(errors)[0];
        const tab = fieldToTab[first];
        if (tab) setActiveTab(tab);
        toast.error(errors[first]?.message || "Please fix the highlighted fields");
    };
                    ...
                    <div className={`sticky top-0 z-10 px-6 py-4 border-b ${theme.border} ${theme.bg}`}>
                        ...
                        <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-2">
                            ...
                            <div className="overflow-y-auto px-6 py-4 flex-1">
                                ...
                                <form id="studentForm" onSubmit={handleSubmit(onSubmit, onError)} className="space-y-6">
