// src/features/apis/finance/ledgerApi.js
import { api } from '../api';

export const ledgerApi = api.injectEndpoints({
    endpoints: (builder) => ({
        getStudentLedger: builder.query({
            query: ({ studentId, startDate, endDate, limit = 200 } = {}) => ({
                url: `/ledger/${studentId}`,
                params: { startDate, endDate, limit },
            }),
            providesTags: (r, e, { studentId }) => [
                { type: 'Ledger', id: studentId },
                'Ledger',
            ],
        }),

        validateLedger: builder.query({
            query: (studentId) => `/ledger/${studentId}/validate`,
            providesTags: (r, e, studentId) => [{ type: 'Ledger', id: `validate-${studentId}` }],
        }),

        getCurrentBalance: builder.query({
            query: (studentId) => `/ledger/${studentId}/balance`,
            providesTags: (r, e, studentId) => [{ type: 'Ledger', id: `balance-${studentId}` }],
        }),
    }),
});
