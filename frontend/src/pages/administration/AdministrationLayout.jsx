          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {adminLinks.map((link) => (
              <NavLink
                ...
                to={link.path}
                className={({ isActive }) =>
                  `block p-6 ${theme.bgCard} rounded-lg border hover:shadow-md transition-shadow ${isActive
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h2 className={`text-lg font-semibold ${theme.text}`}>{link.name}</h2>
                </div>
                <p className={`text-sm ${theme.description}`}>{link.description}</p>
              </NavLink>
            ))}
          </div>
