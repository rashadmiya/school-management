// pages/parent/ParentPaymentResult.jsx
import { Link, useSearchParams } from "react-router-dom";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { useGetParentPaymentIntentQuery } from "@/features/apis/parentPortalApi";
import {
    CheckCircle2, XCircle, Clock, Loader2, ArrowLeft, Download,
} from "lucide-react";
import { formatBDT } from "@/utils/formatCurrency";

export default function ParentPaymentResult() {
    const [params] = useSearchParams();
    const intentId = params.get("intentId");

    const { data, isLoading, isFetching, error } = useGetParentPaymentIntentQuery(
        intentId,
        { skip: !intentId, pollingInterval: 2000 }
    );
            ...
            <div className="max-w-xl mx-auto py-12">
                <Card>
                    <CardContent className="p-8 text-center space-y-4">
                        ...
                        <Button asChild>
                            <Link to="/parent/payments">Back to Payments</Link>
                        </Button>
                    </CardContent>
                </Card>
            </div>
    ...
    if (status === "succeeded") {
        ...
        description = "Your child's fee account has been updated.";
                    ...
                    {intent?.amount && (
                        <div className="pt-4 border-t space-y-2 text-sm">
                            <div className="flex justify-between">
                                ...
                                <span className="font-semibold">{formatBDT(intent.amount)}</span>
                            </div>
